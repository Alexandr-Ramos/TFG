...
ext_in_dev = None
ext_in_ch = None
audio_stream = None  # To keep stream alive
...
freqs = np.fft.rfftfreq(2048, d=1/44100)
line, = ax.plot(freqs, np.zeros_like(freqs))
ax.set_xlim(0, 20000)  # Limit to 20kHz
...
def update_spectrogram(indata, frames, time, status):
    if ext_in_dev is None or ext_in_ch is None:
        return  # Don't update if no device is selected

    # Select the correct channel (convert 1-based index to 0-based)
    audio_data = indata[:, ext_in_ch - 1]

    # Compute FFT
    spectrum = np.abs(np.fft.rfft(audio_data)) / len(audio_data)
    # spectrum = lb.amplitude_to_db(spectrum, ref=np.max)
    spectrum = 20*np.log10(np.abs(spectrum)+1e-4)
...
# Start audio stream
def start_stream():
    global audio_stream  # Keep the stream alive
    if ext_in_dev is not None:
...
def open_settings():
    ...
    def confirm_selection():
        ...
        start_stream()  # Start audio processing
